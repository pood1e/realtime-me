...
install -o root -g root -m 0755 \
  "$REPO_DIR/deploy/library/operator/validate-compose.py" \
  "$LIBEXEC_DIR/validate-compose.py"
install -o root -g root -m 0755 \
  "$REPO_DIR/deploy/library/operator/validate-release-compose.sh" \
  "$LIBEXEC_DIR/validate-release-compose.sh"
install -o root -g root -m 0755 "$REPO_DIR/deploy/library/operator/release.sh" "$SBIN_DIR/cloud-drive-release"
rm -f -- "$SBIN_DIR/cloud-drive-release-api" "$SBIN_DIR/cloud-drive-release-compose"
install -o root -g root -m 0755 "$REPO_DIR/deploy/library/operator/backup-now.sh" "$SBIN_DIR/cloud-drive-backup-now"
install -o root -g root -m 0755 "$REPO_DIR/deploy/library/operator/status.sh" "$SBIN_DIR/cloud-drive-status"
install -o root -g root -m 0755 "$REPO_DIR/deploy/library/operator/logs.sh" "$SBIN_DIR/cloud-drive-logs"
