...
本目录只管理主机上的 PostgreSQL、一次性迁移、Go API、公开路由 Sidecar 与内容 Worker。共享 Tunnel
...
- `api` 加入 `backend` 与独立 `provider-egress`，把 `18081` 分别绑定到主机现有 LAN IP
  和专用 OpenVPN 地址 `10.66.0.11`；不占用新的 `192.168.0.x`，也不加入 edge。
- `library-public` 同时加入 `backend` 与 `realtime-me-edge`，通过精确路由白名单、固定
  public Host 和凭据剥离后才转发到 API；稳定别名为 `library-public`。
...
主机需要 Docker Engine + Docker Compose `5.1.4`、LVM2、rsync、util-linux、openssl 与
...
| `LIBRARY_API_LAN_BIND` | Library 主机现有的静态 LAN IP，必须与 `PRIVATE_API_HOST` 相同 |
| `LIBRARY_API_VPN_BIND` | 固定 overlay 地址 `10.66.0.11` |
| `INTERNAL_API_KEY_FILE` | 统一管理面 32 字节 hex 密钥的 root-only 文件路径 |
...
Library 不再签发浏览器会话。Console BFF 完成 OIDC authorization-code + PKCE 登录，通过
LAN 转发短期 access token，并注入不进入浏览器的内部管理密钥；远程浏览器只有到 Console
这一段经过 OpenVPN。API 独立校验 issuer、audience 与
`PERMISSION_LIBRARY_MANAGE`。匿名 Site 请求只进入显式公开路由。

## Tunnel 与 DNS

Tunnel token 只由 `deploy/edge` 读取，Library 主机配置中不得保留副本。在共享的
remotely-managed named tunnel 中只配置一个 Library Public Hostname：

```text
<PUBLIC_API_HOST>  -> http://library-public:8080
catch-all          -> http_status:404
```

不存在私有 Tunnel hostname。Console 使用 Library 已有 LAN 地址转发 owner access token
和内部密钥；远程维护或采集可使用独立 overlay `10.66.0.11`。浏览器不直接持有二者。

私有 API 的 ConnectRPC、上传、源文件、衍生文件与 Provider callback 均需内部密钥和
有效 OIDC owner token。
公开 API 只挂载壁纸读取、匿名图片和令牌范围内的只读分享路由。
Spotify OAuth callback 注册到 Console 同源 `/api/library/.../callback`；Console 会话、
一次性 state、PKCE、内部密钥和下游 OIDC 校验共同生效。
...
公开 Caddy Sidecar 与本机下载 Worker；只有 Sidecar 可加入名为 `realtime-me-edge` 的外部网络。数据库迁移是
...
`vendor/`、Auth/Library 生成的 Go contracts、`libs/go/authn`、`libs/go/serviceauth`、公开路由
白名单与 Dockerfile 同样由 root
