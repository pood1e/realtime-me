...
export function createPlaybackAdapter(
  ...
  if (descriptor.playback.case === "directAudio") {
    return new DirectAudioPlayer(descriptor.providerId === LOCAL_PROVIDER_ID, events);
  }
  if (descriptor.playback.case === "providerSdk") {
    return createProviderPlayer(descriptor.playback.value.sdkId, client, events);
  }
  return undefined;
}

export function playbackResource(descriptor: PlaybackDescriptor, client: MusicClient): string {
  if (descriptor.playback.case === "directAudio")
    return client.providers.playbackUrl(descriptor.playback.value.url);
  if (descriptor.playback.case === "providerSdk") return descriptor.playback.value.resourceUri;
