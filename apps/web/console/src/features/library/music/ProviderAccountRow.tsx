import { type ProviderConnection, ProviderConnectionStatus } from "@realtime-me/library-contracts";
...
export function ProviderAccountRow({
  ...
  const reconnect = connection.status === ProviderConnectionStatus.RECONNECT_REQUIRED;
  return (
    <div className="flex flex-col gap-4 border-b px-5 py-5 last:border-b-0 sm:flex-row sm:items-center">
      <div className="flex min-w-0 flex-1 items-center gap-4">
        {connection.avatarUrl ? (
          <img src={connection.avatarUrl} alt="" className="size-11 rounded-full object-cover" />
        ) : (
        ...
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            <p className="font-medium">{providerLabel(connection.providerId)}</p>
            <ConnectionBadge status={connection.status} />
          </div>
          <p className="truncate text-sm text-muted-foreground">
            {connection.displayName || providerConnectionDetail(connection.status)}
          </p>
          {connection.membership ? (
            <p className="mt-1 text-xs text-muted-foreground">{connection.membership}</p>
          ) : null}
        </div>
      </div>
      {connected ? (
        ...
        <Button
          onClick={onConnect}
          disabled={connection.status === ProviderConnectionStatus.NOT_CONFIGURED}
        >
