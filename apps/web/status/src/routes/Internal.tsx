...
import { AgentCard, agentKey, EmptyAgentCard } from "@/components/AgentCard";
...
import { authHeaders, isUnauthorized, POLL_INTERVAL_MS, statusClient } from "@/lib/transport";
...
export function InternalStatusApp() {
  ...
  const fetchInternal = useCallback(
    async (signal: AbortSignal) => {
      return (await statusClient.getInternalStatus({}, { headers: authHeaders(token), signal }))
        .status;
    },
    [token],
  );
  ...
  const {
    ...
    error,
    refresh,
  } = usePolling(fetchInternal, { intervalMs: POLL_INTERVAL_MS, enabled: token !== "" });
  ...
  return (
    <Card className="mx-auto w-full max-w-md">
      <CardHeader>
        ...
        <CardDescription>
          {authFailed ? "The saved token was rejected." : "Use the LAN status access token."}
        </CardDescription>
      </CardHeader>
  ...
  return (
    <Tabs defaultValue="overview" className="gap-5">
      ...
      <TabsContent value="metrics">
        <MetricsExplorer status={status} token={token} />
      </TabsContent>
