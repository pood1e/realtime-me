...
export type AppLinks = Partial<Record<PrivateAppId | "wallpapers" | "share", string>>;
...
export function privateAppConfiguration(environment: PrivateAppEnvironment) {
  return {
    apiBase: apiBaseUrl(environment.VITE_PRIVATE_API_BASE, "http://localhost:8080"),
    authOrigin: environment.VITE_AUTH_APP_ORIGIN?.trim() || "http://localhost:5173",
    links: {
      ...
      wallpapers: environment.VITE_WALLPAPERS_APP_ORIGIN || "http://localhost:5178",
