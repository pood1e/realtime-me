...
import { EmptyCard, InlineTime } from "@realtime-me/status-web/components/layout";
import { usePrefersReducedMotion } from "@realtime-me/status-web/hooks/usePrefersReducedMotion";
import {
  agentDeviceLabel,
  subagentCountLabel,
  subagentModelSummary,
} from "@realtime-me/status-web/lib/status";
...
const DEFAULT_MOTION_ASSET: AgentMotionAsset = { src: agentOrbitUrl, durationMs: 4_000 };
const DEFAULT_MOTION_ASSETS: AgentMotionAsset[] = [DEFAULT_MOTION_ASSET];
...
function clawdMotionAssets(): AgentMotionAsset[] {
  return motionAssets(clawdClipUrls, clawdPosterUrls, (name) => CLAWD_CLIP_DURATIONS_MS[name] ?? 0);
}
...
export function AgentClip({
  ...
  const asset = assets[clip.index % assets.length] ?? DEFAULT_MOTION_ASSET;
  const nextAsset = assets[clip.next % assets.length] ?? DEFAULT_MOTION_ASSET;
