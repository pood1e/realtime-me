...
import { agentIcon, agentName } from "@realtime-me/status-web/components/AgentCard";
import { EmptyCard } from "@realtime-me/status-web/components/layout";
import type { ChartPoint, ChartUnit } from "@realtime-me/status-web/lib/format";
import {
  CPU_USAGE,
  hasDiskMetric,
  hasMetric,
  MEMORY_USAGE,
} from "@realtime-me/status-web/lib/metrics";
import { deviceDisplayName, hostDevices } from "@realtime-me/status-web/lib/status";
...
const DEFAULT_CHART_RANGE: ChartRange = {
  id: "1h",
  label: "1h",
...
const StatusChart = lazy(() => import("@realtime-me/status-web/components/StatusChart"));

export function MetricsExplorer({ status, api }: { status: InternalStatus; api: StatusApi }) {
  const [rangeId, setRangeId] = useState(DEFAULT_CHART_RANGE.id);
  const range = CHART_RANGES.find((item) => item.id === rangeId) ?? DEFAULT_CHART_RANGE;
    ...
    <div className="grid gap-4">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        {charts.length === 0 ? (
          ...
          charts.map((chart) => (
            <TimeSeriesCard key={chart.id} chart={chart} range={range} api={api} />
          ))
        )}
      </div>
    </div>
...
function TimeSeriesCard({
  ...
  const { data, failed } = useMetricRange(api, chart, range);
  return (
    <Card>
      <CardHeader>
...
function useMetricRange(
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const response = await api.metrics.getMetricRange(metricRangeRequest(chart, range), {
  ...
  }, [series, deviceUid, agentKind, accessory, range, api]);
...
function chartPoint(point: MetricPoint): ChartPoint | null {
