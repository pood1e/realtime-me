import type { GithubSyncState, MobileState, WatchSnapshot } from "@realtime-me/status-contracts";
import { ChargeState, OnlineState } from "@realtime-me/status-contracts";
import { AccessoryBadges, MetricBadges, StatCell } from "@realtime-me/status-web/components/badges";
import { BrandIcon } from "@realtime-me/status-web/components/brand";
import { GitHubStatusBadge } from "@realtime-me/status-web/components/github";
import { InlineTime } from "@realtime-me/status-web/components/layout";
import { SwitchArtwork } from "@realtime-me/status-web/components/SwitchArtwork";
import { formatBattery } from "@realtime-me/status-web/lib/format";
import { isPlayingOnSwitch, networkLabel } from "@realtime-me/status-web/lib/status";
...
export function MobileDeviceCards({
  mobiles,
  githubState,
}: {
  mobiles: MobileState[];
  githubState?: GithubSyncState | undefined;
}) {
  return (
    <>
      {mobiles.map((mobile) => (
        <PhoneCard key={`phone:${mobile.deviceUid}`} mobile={mobile} />
      ))}
      {mobiles
        .filter((mobile) => mobile.watch)
        .map((mobile) => (
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          <BrandIcon icon={siAndroid} />
          {displayName}
        </CardTitle>
        <CardAction className="flex items-center gap-2">
          <InlineTime value={mobile?.updateTime} />
          {mobile ? (
            <Badge aria-label="Online">
              <CheckCircle2 />
            </Badge>
          ) : (
            <Badge variant="outline" aria-label="No data">
              —
            </Badge>
          )}
        </CardAction>
      ...
      <CardContent className="flex h-full flex-col gap-4">
        <div className="flex flex-wrap items-start justify-around gap-x-2 gap-y-3 py-1">
          ...
          <StatCell
            icon={<Wifi />}
            value={phone ? networkLabel(phone.network) : "—"}
            label="Network"
          />
        </div>
    ...
    <Card>
      ...
      <CardContent className="flex h-full flex-col gap-4">
        <div className="flex flex-wrap items-start justify-around gap-x-2 gap-y-3 py-1">
          ...
          <StatCell
            icon={
              watchState?.chargeState === ChargeState.CHARGING ? <BatteryCharging /> : <Battery />
            }
  ...
  return (
    <Card>
      ...
      <CardContent className="flex h-full flex-col gap-4">
        ...
        <div className="flex flex-wrap items-start justify-around gap-x-2 gap-y-3 py-1">
          {!gameName && (
            <StatCell icon={<Gamepad2 />} value={online ? "Online" : "—"} label="Presence" />
          )}
          <StatCell
            icon={<CheckCircle2 />}
            value={presence ? onlineStateLabel(presence.state) : "—"}
            label="State"
          />
        </div>
        {!!gameName && !!presence?.titleId && (
          <MetricBadges>
            <Badge variant="secondary" title={presence.titleId}>
              Title {presence.titleId}
            </Badge>
          </MetricBadges>
        )}
      </CardContent>
...
function WatchStatusBadge({ watch }: { watch?: WatchSnapshot | undefined }) {
  if (!watch)
    return (
      <Badge variant="outline" aria-label="No data">
        —
...
function SwitchStatusBadge({ mobile }: { mobile: MobileState | null }) {
  ...
  if (presence.state === OnlineState.ONLINE)
    return (
      <Badge aria-label="Online">
        <CheckCircle2 />
      </Badge>
  ...
  return (
    <Badge variant="outline" aria-label="Offline">
      Offline
    </Badge>
  );
}
