...
func (s *Store) CompleteTrackProcessing(ctx context.Context, track domain.Track, artwork *domain.Artifact) error {
	...
	_, err = tx.Exec(ctx, `UPDATE tracks SET title = CASE WHEN $2 <> '' THEN $2 ELSE title END,
		...
		track.UID, track.Title, track.Artists, track.Album, track.AlbumArtist, track.TrackNumber,
