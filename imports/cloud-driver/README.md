...
或 MinIO。音乐盒可以按需连接外部会员账号，但不下载或缓存第三方音频。
...
Provider 是编译期插件：Registry 以 `MusicProvider` 注册 Adapter，上层只依赖登录、
二维码轮询、OAuth 回调、目录搜索、播放解析、歌词和 SDK Token 等独立能力接口。
不支持某项能力的平台无需提供空实现；新增平台不会修改现有 Adapter 的业务代码。
...
- 第三方音乐只保存加密账号凭据与播放历史快照，不保存搜索结果、播放 URL 或音频。
...
| `web/apps/music` | 本地音乐、会员来源搜索、双播放引擎、歌词与统一历史 |
