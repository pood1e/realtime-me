...
require_root
(($# == 1)) || die 'usage: cloud-drive-logs api|worker|postgres|cloudflared|backup'
case "$1" in
  api|worker|postgres|cloudflared)
  ...
  backup)
    ...
    ;;
  *)
    die 'usage: cloud-drive-logs api|worker|postgres|cloudflared|backup'
    ;;
esac
