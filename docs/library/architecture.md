...
API、Worker、Migrate 与 PostgreSQL 不向宿主机发布端口。`cloudflared` 只能通过 Compose 的
...
| `web/apps/music` | 本地音乐、会员来源搜索、歌单导入/下载、双播放引擎、歌词与统一历史 |
...
生成代码漂移检查、Prettier/typecheck/build、Shell 语法检查与 ShellCheck。部署前还应渲染 Compose
